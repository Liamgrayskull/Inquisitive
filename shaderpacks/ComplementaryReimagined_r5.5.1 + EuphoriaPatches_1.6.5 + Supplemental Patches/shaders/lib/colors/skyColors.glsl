    ...
    #define INCLUDE_SKY_COLORS
        ...
        vec3 noonMiddleSkyColor = skyColorM * (vec3(1.15) + rainFactorM * (ndscWeatherM + ndscRainM + ndscSnowM + ndscDryM + sandstormM))
                                + noonUpSkyColor * 0.6;
        vec3 noonDownSkyColor   = skyColorM * (vec3(0.9) + rainFactorM * (ndscWeatherM + ndscRainM + ndscSnowM + ndscDryM + sandstormM))
                                + noonUpSkyColor * 0.25;

        vec3 sunsetUpSkyColor     = skyColorM2 * (vec3(0.8, 0.58, 0.58) + vec3(0.1, 0.2, 0.35) * rainFactorM2);
        ...
        vec3 nightColFactor      = vec3(0.07, 0.14, 0.24) * (1.0 - 0.5 * rainFactorM) + skyColor + 0.4 * rainFactorM * sandstormM;
